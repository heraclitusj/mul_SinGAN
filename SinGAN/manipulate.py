...
def SinGAN_generate(Gs,Zs,reals,NoiseAmp,opt,in_s=None,scale_v=1,scale_h=1,n=0,gen_start_scale=0,num_samples=50):
    ...
    for G,Z_opt,noise_amp in zip(Gs,Zs,NoiseAmp):
        ...
        for i in range(0,num_samples,1):
            ...
            z_in = noise_amp*(z_curr)+I_prev
            if opt.input_name == opt.input_name1:
                labels_onehot = np.array([[1.0,0.0]])
            if opt.input_name == opt.input_name2:
                labels_onehot = np.array([[0.0,1.0]])
            real_label = Variable(torch.from_numpy(labels_onehot).float()).cuda() 
            real_label = real_label.unsqueeze(2).unsqueeze(3)
            real_label = real_label.repeat(1,1,z_curr.shape[2],z_curr.shape[3])
            z_in = torch.cat((z_in,real_label),dim=1)
            I_curr = G(z_in.detach(),I_prev)
