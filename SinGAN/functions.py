...
def read_image(opt):
    ...
    x = img.imread('%s/%s' % (opt.input_dir,opt.input_name1)) #250*250*4
    y = img.imread('%s/%s' % (opt.input_dir,opt.input_name2)) #250*250*4
    x = np2torch(x,opt)
    y = np2torch(y,opt)
    x = x[:,0:3,:,:] #变为3通道 
    y = y[:,0:3,:,:] #变为3通道
...
def np2torch(x,opt):
    if opt.nc_im == 3:
        x = x[:,:,:,None]
        x = x.transpose((3, 2, 0, 1))/255 #1,channel,h,w 1*4*250*250
    else:
        x = color.rgb2gray(x)
        x = x[:,:,None,None]
...
def save_networks(netG,netD,z,opt):
    torch.save(netG.state_dict(), '%s/netG.pth' % (opt.outf1))
    torch.save(netD.state_dict(), '%s/netD.pth' % (opt.outf1))
    torch.save(z, '%s/z_opt.pth' % (opt.outf1))
    torch.save(netG.state_dict(), '%s/netG.pth' % (opt.outf2))
    torch.save(netD.state_dict(), '%s/netD.pth' % (opt.outf2))
    torch.save(z, '%s/z_opt.pth' % (opt.outf2))

def adjust_scales2image(real_,opt):
    #opt.num_scales = int((math.log(math.pow(opt.min_size / (real_.shape[2]), 1), opt.scale_factor_init))) + 1
    # log(最底层输入大小/真实图片大小, 初始scale权重) + 1 
    ...
    # 最后一层生成的图片应该小于设定的最大图片尺寸
    scale2stop = math.ceil(math.log(min([opt.max_size, max([real_.shape[2], real_.shape[3]])]) / max([real_.shape[2], real_.shape[3]]),opt.scale_factor_init))
    # 在满足最后生成图片小于最大尺寸的情况下， 从最底层输入大小到最后一层输出大小需要多少层。pyramid_layer_num
    opt.stop_scale = opt.num_scales - scale2stop
    # scale1: 保证图像尺寸不大于opt.max_size
    opt.scale1 = min(opt.max_size / max([real_.shape[2], real_.shape[3]]),1)  # min(250/max([real_.shape[0],real_.shape[1]]),1)
    # resize真实图片， 插值部分作者自己写的，很有意思。real = imresize(real_, opt.scale1, opt)
...
def load_trained_pyramid(opt, mode_='train'):
    ...
    if (mode == 'animation_train') | (mode == 'SR_train') | (mode == 'paint_train'):
        opt.mode = mode
    dir,dir1,dir2 = generate_dir2save(opt)
    print(dir1)
    if(os.path.exists(dir1)):
        Gs = torch.load('%s/Gs.pth' % dir1)
        Zs = torch.load('%s/Zs.pth' % dir1)
        reals = torch.load('%s/reals.pth' % dir1)
        NoiseAmp = torch.load('%s/NoiseAmp.pth' % dir1)
    else:
...
def generate_dir2save(opt):
    ...
    if (opt.mode == 'train') | (opt.mode == 'SR_train'):
        dir2save1 = 'TrainedModels/%s/scale_factor=%f,alpha=%d' % (opt.input_name1[:-4], opt.scale_factor_init,opt.alpha)
        dir2save2 = 'TrainedModels/%s/scale_factor=%f,alpha=%d' % (opt.input_name2[:-4], opt.scale_factor_init,opt.alpha)
        ...
        dir2save1 = '%s/RandomSamples_ArbitrerySizes/%s/scale_v=%f_scale_h=%f' % (opt.out,opt.input_name1[:-4], opt.scale_v, opt.scale_h)
        dir2save2 = '%s/RandomSamples_ArbitrerySizes/%s/scale_v=%f_scale_h=%f' % (opt.out,opt.input_name2[:-4], opt.scale_v, opt.scale_h)
    ...
    return dir2save,dir2save1,dir2save2

def post_config(opt):
    # init fixed parameters
    opt.device = torch.device("cpu" if opt.not_cuda else "cuda:0")
    opt.niter_init = opt.niter #2000
    opt.noise_amp_init = opt.noise_amp #0.1
    opt.nfc_init = opt.nfc #32,初始特征数
    opt.min_nfc_init = opt.min_nfc #32
    opt.scale_factor_init = opt.scale_factor #0.75
    opt.out_ = 'TrainedModels/%s/scale_factor=%f/' % (opt.input_name1[:-4], opt.scale_factor)
    opt.out_ = 'TrainedModels/%s/scale_factor=%f/' % (opt.input_name2[:-4], opt.scale_factor)
    if opt.mode == 'SR':
        opt.alpha = 100
